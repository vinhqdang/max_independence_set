...
All runs are performed on the same machine and are single-threaded. Every solver
...
Baselines are built from their authors' published sources. NuMVC and FastVC are
taken from a library that carries the original implementations; they solve the
minimum vertex cover formulation, and we convert their output to an independent
set before verification.
...
On the geometric family \textsc{Cascade} is ahead on every instance, and the
margin grows with size: $+10$ on \texttt{del16}, $+277$ on \texttt{del18} and
$+5535$ on \texttt{del20}, the last being $1.7\%$ of the best value any other
solver reached. This is the family whose kernels remain a large fraction of the
input, so it is where the choice of what to do after kernelization matters most,
and it is where a move that restructures tens of vertices at once has the most
to work with. On the random geometric graphs the reduction rules are far more
effective and several solvers reach the same value, which the exact solvers
confirm is optimal for \texttt{rgg16} and \texttt{rgg18}.

On road networks and on most social networks the strong solvers agree to within
a handful of vertices, because the kernels are tiny: \texttt{com-lj} has four
million vertices and a kernel of 1781. What separates the solvers there is
robustness rather than quality. ReduMIS returned nothing on \texttt{soc-pokec}
within eleven minutes, and NuMVC returned nothing on \texttt{com-lj} within
sixteen; \textsc{Cascade} produced a verified solution on every instance.

On web graphs \textsc{Cascade} is behind ReduMIS by 15 and 100 vertices, which
is $0.01\%$ and $0.02\%$. On the BHOSLIB family it reaches the known optimum on
several instances but is one vertex short on others, where NuMVC is both better
and dramatically faster --- it attains the optimum of \texttt{frb30-15-1} in
40\,ms, against a full budget for the reduction-based solvers. The structural
...
$\tau = 1$ means it matched the best. A curve that never reaches $1.0$ had a run
that returned no solution at all: ReduMIS on \texttt{soc-pokec} and NuMVC on
\texttt{com-lj}. Note that the profile is a \emph{relative} measure and
therefore weights the small dense instances heavily, where losing a single
vertex is already a $2$--$3\%$ ratio; Table~\ref{tab:aggregate} and the absolute
margins below give the complementary view.}
\label{fig:profile}
\end{figure}
...
finds no significant difference against ReduMIS ($p = 0.195$). Summed over the
instances both solved, \textsc{Cascade} is ahead of ReduMIS by 8651 vertices and
behind by 122: where ReduMIS wins it wins by one or two vertices, and where
\textsc{Cascade} wins it wins by thousands.

Figure~\ref{fig:profile} shows the same data as a performance profile.
\textsc{Cascade} matches the best value on the largest share of instances among
the solvers that never fail, and it is one of only three that reach every
instance at all. Its curve is flat between $\tau \approx 1.001$ and
$\tau \approx 1.017$, which is the BHOSLIB family: a one-vertex shortfall on a
35-vertex optimum is a $2.9\%$ ratio, so those instances sit far to the right
even though the absolute loss is one vertex each.
...
our solver's five runs collapse to a line: the spread is below $0.05\%$, and on
several instances all five seeds return the same value. On the BHOSLIB
instances it is the widest of any solver, reaching $2.5\%$ on
\texttt{frb40-19-1}. This is the same degeneracy discussed in
