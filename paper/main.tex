...
cover linear program is half-integral and that its integral part can be fixed.
Exact exponential algorithms contributed the local rules now used in practice,
including the unconfined and confining-set machinery of Xiao and Nagamochi
\cite{xiao2017}. Akiba and Iwata \cite{akiba2016} first demonstrated that a
branch-and-reduce algorithm carrying the full rule set is competitive with, and
often superior to, integer programming on real instances. Hespe et al.
\cite{hespe2019} parallelised kernelization and characterised which instance
families reduce well. Gellner et al. \cite{gellner2021} showed that rules which
temporarily \emph{increase} the graph can unlock further reduction.
...
with two-stage exchange and configuration checking and is extremely strong on
dense benchmark instances, while FastVC \cite{cai2017fastvc} trades some of that
quality for the ability to run on massive sparse graphs at all.
...
instances and is a component of the solver that won the PACE 2019 vertex cover
challenge \cite{hespe2020}.

\paragraph{Positioning.} Large-neighbourhood search with an exactly solved
sub-problem is a standard metaheuristic pattern. What distinguishes the present
work is the interaction with data reductions: the region is defined so that it
is provably separated from the incumbent, and the reduction rules are what make
solving it exactly cheap enough to sit in the inner loop. The neighbourhood size
is then not a parameter but a quantity inferred at run time from the difficulty
of the sub-problems.
