...
solvers or of the machine, and on that the answer is unambiguous: on every
instance, the set of solvers attaining the best value on the replication machine
intersects the set attaining it on the original machine. No solver changes place
with another. A reader can therefore take the comparisons in
...
\emph{identical} values on all instances, which is the control: it confirms that
...
reduction phase to fall back on. And a fixed wall-clock budget on two machines
