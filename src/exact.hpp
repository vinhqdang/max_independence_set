...
class ExactSolver {
public:
    // Reuses its buffers across calls: the search runs thousands of small
    // regions a second, and rebuilding the graph and reduction state each time
    // costs more than the search itself.
    ExactResult solve(const Graph& g, const ExactConfig& cfg, double deadline,
...
// Convenience wrapper for one-off solves.
inline ExactResult solve_exact(const Graph& g, const ExactConfig& cfg, double deadline,
                               const std::function<double()>& elapsed) {
    if (g.n == 0) {
        ExactResult r;
    ...
    ExactSolver s;
    return s.solve(g, cfg, deadline, elapsed);
}
